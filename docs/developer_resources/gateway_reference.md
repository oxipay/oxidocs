...
Communicating with Oxipay in order to process a transaction via our payment gateway involves **the buyers browser** peforming an HTTP **POST** to the Oxipay endpoint. This can be viewed as an authorisation request that is performed by the shopping cart in order to process the payment via Oxipay.
